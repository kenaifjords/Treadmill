%% initiate treadmill command connection
% only run this once after turning the treadmill on
prompt = {'Do you want to start the TCP/IP connection with the treadmill(Y/N)? (only run this once after turning on the treadmill)'};
dlg_title = 'Input';
num_lines = 1;
defaultans = {'N'}; %DEFAULT ANSWERS
...
prompt = {'Subject Name','Enter fast leg (L,R,none):',...
    'Enter Body Weight (lb):','Enter leg length (cm):',...
    'Enter Effort Level (high/low/none):'};
dlg_title = 'Input';
num_lines = 1; 
defaultans = {'test00','none','130','80','none'}; %DEFAULT ANSWERS
answer = inputdlg(prompt,dlg_title,num_lines,defaultans);
% assign input answers to variables
s.subj = answer{1};
s.fastleg = answer{2};
s.bodyweight = str2double(answer{3})*0.453592; % kg
s.leglength = str2double(answer{4});
s.effortlevel = answer{5};

speed_init = 0.5; %m/s
center = 0.9; % m
% set safe acceleration limits
s.accel = 0.1; % m/s^2
%% Start the Vicon Datastream
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% formerly section to check the existence of bReadCentroids, bReadRays,
% bTrajectoryIDs, axisMapping, bLightweightSegment and printed them out
...
%% start treadmill
speed = speed_init;
tm_set(remote, speed, 0.1); 
disp('start treadmill')
t_sendPacket = tic;

%% start data stream loop
...
while ishandle( MessageBox )
  ...
  % removed if EnableHapticFeedbackTest loop      

  % Get the frame number
  Output_GetFrameNumber = MyClient.GetFrameNumber();
  if Frame ~= -1
    % identify any skipped frames
  ...
  framedata.timecode(loop_count) = double(Output_GetTimecode.Standard);
  framedata.time_subframes(loop_count) = Output_GetTimecode.SubFramesPerFrame;
  
  % Get the latency
  fprintf( 'Latency: %gs\n', MyClient.GetLatencyTotal().Total );
  framedata.time_latency(loop_count) = MyClient.GetLatencyTotal().Total;
  ...
  framedata.hardwareFrameNum(loop_count) = Output_GetHardwareFrameNumber.HardwareFrameNumber;
  ...
  fprintf( 'Subjects (%d):\n', SubjectCount );
  % everything inside this loop is commented out - 
...
%     
  ...
  figure(1)
  cla
  % everything inside this loop is commented out since it deals with
  % markers only
  for MarkerIndex = 0:typecast( MarkerCount, 'int32' )-1
%     % Get the marker name
%     MarkerName = MyClient.GetMarkerName( SubjectName, typecast( MarkerIndex,'uint32') ).MarkerName;
...
%     Output_GetMarkerGlobalTranslation = MyClient.GetMarkerGlobalTranslation( SubjectName, MarkerName );
% 
%     plot3(Output_GetMarkerGlobalTranslation.Translation( 1 ), ...
%     Output_GetMarkerGlobalTranslation.Translation( 2 ), ...
%     Output_GetMarkerGlobalTranslation.Translation( 3 ),'bo');
%    %        -x    x    -y     y    -z     z
%     axis([-2000 2000 -1500 1500 -1200 1200])
% 
%     hold on
%   end
...
%     end
  ...
  fprintf( '  Devices (%d):\n', DeviceCount );
  framedata.deviceCount(loop_count) = DeviceCount;
  
  % this loop is set up to save device outputs, but the whole loop is
  % commented out since force plates are not devices (?)
...
%     framedata.device(DeviceIndex).Name(loop_count) = char( Output_GetDeviceName.DeviceName );
%     framedata.device(DeviceIndex).Type(loop_count) = char( Output_GetDeviceName.DeviceType.ToString() );
%     
...
%       framedata.device(DeviceIndex).output(DeviceOutpuIndex).name(loop_count) = Output_GetDeviceOutputComponentName;
% 
...
%       framedata.device(DeviceIndex).output(DeviceOutpuIndex).subsamples(loop_count) = Output_GetDeviceOutputSubsamples;
% 
...
%         framedata.device(DeviceIndex).output(DeviceOutputIndex).sample(DeviceOutputSubsample).data(loop_count) = Output_GetDeviceOutputValue;
%         fprintf( '          ''%s'' %g %s %s\n',                                    ...
...
%     end% DeviceOutputIndex
  ...
  for ForcePlateIndex = 0:typecast( ForcePlateCount, 'int32' ) -1
    ...
    % Create emptyy matrix for subsample
    if ForcePlateIndex==0
        Output_GetGlobalCentreOfPressureAll_1= zeros(Output_GetForcePlateSubsamples.ForcePlateSubsamples+1,3);
        Output_GetGlobalForceVectorAll= zeros(Output_GetForcePlateSubsamples.ForcePlateSubsamples+1,3,2);
    else
        Output_GetGlobalCentreOfPressureAll_2= zeros(Output_GetForcePlateSubsamples.ForcePlateSubsamples+1,3);
    end
    fprintf( '    Samples (%d):\n', Output_GetForcePlateSubsamples.ForcePlateSubsamples );
    framedata.forceplate(ForcePlateIndex).force(loop_count,:) = Output_GetForcePlateSubsamples.ForcePlateSubsamples;
...
%                          Output_GetGlobalCentreOfPressure.CentreOfPressure( 3 ) ); 
      framedata.forceplate(ForcePlateIndex).CoP(loop_count,:) = Output_GetGlobalCentreOfPressure.CentreOfPressure;
      
      % Store samples
      if ForcePlateIndex==0
           Output_GetGlobalCentreOfPressureAll_1(ForcePlateSubsample+1,:) = [Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ) Output_GetGlobalCentreOfPressure.CentreOfPressure( 2 ) Output_GetGlobalCentreOfPressure.CentreOfPressure( 3 )];
           Output_GetGlobalForceVectorAll(ForcePlateSubsample+1,:,1) =[Output_GetGlobalForceVector.ForceVector( 1 ) Output_GetGlobalForceVector.ForceVector( 2 ) Output_GetGlobalForceVector.ForceVector( 3 )];
      else
           Output_GetGlobalCentreOfPressureAll_2(ForcePlateSubsample+1,:) = [Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ) Output_GetGlobalCentreOfPressure.CentreOfPressure( 2 ) Output_GetGlobalCentreOfPressure.CentreOfPressure( 3 )];
           Output_GetGlobalForceVectorAll(ForcePlateSubsample+1,:,2) =[Output_GetGlobalForceVector.ForceVector( 1 ) Output_GetGlobalForceVector.ForceVector( 2 ) Output_GetGlobalForceVector.ForceVector( 3 )];
      end
      figure(1)
      if ForcePlateIndex==0
        plot(Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ),Output_GetGlobalCentreOfPressure.CentreOfPressure( 2 ),'rx')
      else
        plot(Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ),Output_GetGlobalCentreOfPressure.CentreOfPressure( 2 ),'bx')
      end
      axis([0 1 0 2])
      hold on
      ...
      figure(2)
      if ForcePlateIndex==0
        plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ),'rx')
      else
        plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,Output_GetGlobalCentreOfPressure.CentreOfPressure( 1 ),'bx')
      end
      axis([0 200 0 1])
      hold on
    ...
    figure(4)
    if ForcePlateIndex==0
        plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,mean(Output_GetGlobalForceVectorAll( :,3,ForcePlateIndex+1 )),'rx')  
    else
        plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,mean(Output_GetGlobalForceVectorAll( :,3,ForcePlateIndex+1 )),'bx')  
    end
    %axis([0 200 0 200])
    hold on
  end% ForcePlateIndex
  ...
  if ForcePlateIndex>=0  
    figure(3)
    plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,mean(Output_GetGlobalCentreOfPressureAll_1( :,1 )),'rx')
    hold on
    plot(Output_GetFrameNumber.FrameNumber/Output_GetFrameRate.FrameRateHz,mean(Output_GetGlobalCentreOfPressureAll_2( :,1 )),'bx')
    axis([0 200 0 1])
    hold on
  end
...
%     end% CameraIndex
  ...
  %% SPEED CONTROL
  if toc(t_sendPacket) > 500
      if CoP_forSpeed(2) > center + 0.05
        new_speed = speed + 0.2; % m/s
      elseif CoP_forSpeed(2) < center - 0.05
        new_speed = speed - 0.2; % m/s
      else
         new_speed = speed;
      end
      speed = new_speed;
      tm_set(remote, new_speed, s.accel); 
      disp(num2str(speed))
      t_sendPacket = tic;
      disp('SENT NEW SPEED')
      disp(new_speed)
