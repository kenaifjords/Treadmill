function [alignedMatrix,centerI] = alignProfiles3(trialProfiles,...
    alignIndices, desired_width)
...
%             end
    ...
    % preassign matrix
    width_with_tails = ptail + tail + 2; % allows for a nan pad on longest 
    % trial
    % compare width to desired width
    if width_with_tails > desired_width
        aM = nan(tricount, width_with_tails);
        disp('Requested width is too narrow to align profiles')
        tack_on = 0;
    elseif width_with_tails == desired_width
        aM = nan(tricount, width_with_tails);
        tack_on = 0;
    else
        aM = nan(tricount, desired_width);
        tack_on = desired_width - width_with_tails
    end
    ...
    for j = 1:tricount
            if isnan(I(j))
                ...
                ppad = ptail - I(j) + 1;
                pad = tail - (lengthProfile(j) - I(j)) + 1 + tack_on;
                prepad = nan(1,ppad);
