% TM2figure_plotTimeAsymmetry
% plots step time learning curves against time
function TM2figure_plotTimeAsymmetry
close all
...
for subj = 1:subject.n
    for effcond = 1:length(subject.effortcondition)
        for blk = 1:subject.nblk
            ...
            plot(hsLasym,steptime1,'r.')
%             title('Step Lengths')
            hold on
            plot(hsLasym,steptime2,'g.')
...
%             ylim([300,800]);
            ...
            plot(hsLasym,steptime_asym,'Color',colorasym)
...
%             asym(subj).steplength_r{effcond,blk} = steptime1;
%             asym(subj).steplength_l{effcond,blk} = steptime2;
%             asym(subj).nsteps{effcond,blk} = maxsteps;
%             asym(subj).asymlength{effcond,blk} = steptime_asym;
...
%                 ylim([300,800]);
            ...
            if blk == 4 || blk == 6 % split 1 %split 2 = saving
                if blk == 4
                    subplot(221); hold on;
                    plot(hsLasym,steptime_asym,'Color',colorasym,'LineWidth',1.5)
                    ylabel('Asymmetry (fast - slow) Length (mm)')
                    ylim([-0.5 0.5])
                    xlim([0 615])
                    ...
                    subplot(222); hold on;
                    plot(hsLasym,steptime_asym,'Color',colorasym,'LineStyle',':','LineWidth',2)
                    ylim([-0.5 0.5])
                    xlim([0 615])
