...
%% identify trial metrics
clear fmet_phase
fnames = {'pkf_f' 'pkf_s' 'minf_f' 'minf_s' 'impf' 'imps'};
for subj = 1:subject.n
    for effcond = 1:size(F(subj).R,1)
        for blk = 1:size(F(subj).R,2)
            if ~isempty(F(subj).R{effcond,blk})
                % concatenate into a 3d matrix stride x profile x direction
                sdf = cat(3,F(subj).strideforce(1).fast{effcond,blk},...
                    F(subj).strideforce(2).fast{effcond,blk},...
                    F(subj).strideforce(3).fast{effcond,blk});
                sds = cat(3,F(subj).strideforce(1).slow{effcond,blk},...
                    F(subj).strideforce(2).slow{effcond,blk},...
                    F(subj).strideforce(3).slow{effcond,blk});
                impf = F(subj).impulsefast{effcond,blk};
                imps = F(subj).impulseslow{effcond,blk};
             
                % peak force (from each profile (row))x, y, x, this should
                % result in a stride x 1 x direction matrix, use squeeze to
                % make it stride x direction
                
                % fast
                % peak lateral (outward), peak propulsive, peak z
                pkf_f = squeeze(max(sdf,[],2,'omitnan'));
                % min lateral (inward), peak braking, min z(should be 0);
                minf_f = squeeze(min(sdf,[],2,'omitnan'));
                % impulse is already stored in impf
                impf = impf';
                
                % slow
                % peak lateral (outward), peak propulsive, peak z
                pkf_s = squeeze(max(sds,[],2,'omitnan'));
                % min lateral (inward), peak braking, min z(should be 0);
                minf_s = squeeze(min(sds,[],2,'omitnan'));
                % impulse is already stored in imps
                imps = imps'; 
                
                % match sizes in case fast or slow has more steps
                if size(pkf_f,1) ~= size(pkf_s,1)
                    sdif = size(pkf_f,1) - size(pkf_s,1);
                    if sdif > 0
                        sadd = NaN(sdif,3);
                        pkf_s = cat(1,pkf_s,sadd);
                        minf_s = cat(1,minf_s,sadd);
                        %imps = cat(1, imps', NaN(sdif,1));
                        %impf = impf';
                    else
                        sadd = NaN(-sdif,3);
                        pkf_f = cat(1,pkf_f,sadd);
                        minf_f = cat(1,minf_f,sadd);
                        %impf = cat(1,impf',NaN(sdif,1));
                        %imps = imps';
                    end
                end
                % match impulse length
                if size(pkf_f,1) > size(imps,1)
                    sdif = size(pkf_f,1) - size(imps,1);
                    if sdif > 0
                        imps = cat(1,imps,NaN(sdif,1));
                    end
                end
                if size(pkf_f,1) > size(impf,1)
                    sdif = size(pkf_f,1) - size(impf,1);
                    if sdif > 0
                        impf = cat(1,impf,NaN(sdif,1));
                    end
                end
                % get force asymmetry for other use
                pkfasym = (pkf_f - pkf_s) ./ (pkf_f + pkf_s);
                minfasym = (minf_f - minf_s) ./ (minf_f + minf_s);
                impasym = (impf - imps) ./ (impf + imps);
%                 pkforceasym{subj,effcond,blk} = pkfasym;
%                 minforceasym{subj,effcond,blk} = minfasym;
                
                % store into structure for looping through the phase and
                % variability function, this generates a stride x metrics
                % and directiosn
                fmet = cat(2,pkf_f,pkf_s,minf_f,minf_s,impf,imps,...
                    pkfasym,minfasym,impasym);
                % sort into phases and get variability
                for i = 1:size(fmet,2)
                    [fmet_p0, fmet_pv0] = ...
                        getPhaseMeanAndVariability(fmet(:,i));
%                     [fsym_pkphase0, fasym_pkpvar0] = ...
%                         getPhaseMeanAndVariability(pkfasym(:,i));
%                     [fsym_minphase0, fasym_minpvar0] = ...
%                         getPhaseMeanAndVariability(minfasym(:,i));
                    % store (subj, effcond, blk, metric, phase) #yikes
                    fmet_phase(subj,effcond,blk,i,:) = fmet_p0;
                    fmet_phasevar(subj,effcond,blk,i,:) = fmet_pv0;
%                     pkfasym_phase(subj,effcond,blk,i,:) = fasym_pkphase0;
%                     pkfasym_phasevar(subj,effcond,blk,i,:) = fasym_pkpvar0;
%                     minfasym_phase(subj,effcond,blk,i,:) = fasym_minphase0;
%                     minfasym_phasevar(subj,effcond,blk,i,:) = fasym_minpvar0;
                end
            end
        end
    end
...
%% save variable names
fmet_names = {"pkfx_f" "pkpush_f" "pkfz_f" "pkfx_s" "pkpush_s" "pkfz_s" ...
    "minfx_f" "pkbrake_f" "minfz_f" "minfx_s" "pkbrake_s" "minfz_s"...
    "imp_f" "imp_s" "pkx_asym" "pky_asym" "pkz_asym" "minx_asym"...
    "miny_asym" "minz_asym" "imp_asym"};
for i = 1:length(fmet_names)
    fmet_varnames{i} = strcat(fmet_names{i},"var");
end
%% save fmet phases
save('fmet_phase','fmet_phase')
save('fmet_phasevar','fmet_phasevar')
% save('peakforceasym','pkfasym')
% save('minimumforceasym','minfasym')
% save('pkforceasym_phase','pkfasym_phase');
% save('minforcesaym_phase','minfasym_phase')
%% make R file
if 0
    a = load('fmet_phase.mat'); b = load('fmet_phasevar.mat');
    c = load('subject_structure.mat')
    subject = c.subject;
    fmet_phase = a.fmet_phase;
    fmet_phasevar = b.fmet_phasevar;
end
make_Rfmet_file(fmet_phase,fmet_phasevar,fmet_names,fmet_varnames,...
    subject.order);
