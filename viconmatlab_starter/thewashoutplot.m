...
if exist('firstplat','var')
    ...
    %         plot(fp,-0.2-i/20*ones(length(fp),1),'.','Color',colors.all{i,1},'MarkerSize',5);
            ...
            plot(mean(fp,'omitnan'),0.2+i/20,'s','Color',colors.all{k,1},'MarkerSize',10,'HandleVisibility','off');
            m = mean(fp,'omitnan');
            range = m + [-std(fp,[],'omitnan') std(fp,[],'omitnan')]./sqrt(length(fp));
            plot(range, 0.2+i/20*ones(2,1),'Color',colors.all{k,1},'HandleVisibility','off');
        end
    end
end
