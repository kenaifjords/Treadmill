function [spark,spark_color] = sortbyEffortVisitorder_spark2(asym_allsubj)
global subject
...
for grp = [4 1 5 2 3] %1:5
    for subj = 1:subject.n
        if subject.twovisitgroup(subj) == grp
            ...
            eval(['count' num2str(subord) ' = count' num2str(subord) ' + 1;'])
            % learning
            spark{subord,eval(['count' num2str(subord)])} = asym_allsubj{subord,4}(subj,:);
            spark_color(subord,eval(['count' num2str(subord)])) = subord;
%             % relearning
