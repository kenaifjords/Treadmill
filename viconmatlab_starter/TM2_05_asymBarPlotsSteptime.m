...
for subj = 1:subject.n
    for blk = 1:subject.nblk
        if subject.order(subj) ~= [0 0]% 1 and 2 here refer to order of visits
            ...
            learncurvesC_cell{ctrlcount,blk} = asym(subj).steptime{3,blk};
        end
    end
...
initialpert_subj = [mean(highsplit1(:,initialpertsteps),2,'omitnan'), mean(lowsplit1(:,initialpertsteps),2,'omitnan'),...
    mean(highsplit2(:,initialpertsteps), 2,'omitnan'), mean(lowsplit2(:,initialpertsteps), 2,'omitnan'),...
    mean(highsplit3(:,initialpertsteps), 2,'omitnan'), mean(lowsplit3(:,initialpertsteps), 2,'omitnan'),...
    mean(highsplit4(:,initialpertsteps), 2,'omitnan'), mean(lowsplit4(:,initialpertsteps), 2,'omitnan')];
...
figure(); hold on;
clear colr
colr(1,1,:) = colors.high2; colr(1,2,:) = colors.low2;
colr(2,1,:) = colors.high2; colr(2,2,:) = colors.low2; 
colr(3,1,:) = colors.low2; colr(3,2,:) = colors.high2;
colr(4,1,:) = colors.low2; colr(4,2,:) = colors.high2;
edgcolr = nan(4,2,3);
edgcolr(1,1,:) = [colors.high2]; edgcolr(1,2,:) = [colors.low2];
edgcolr(2,1,:) = [colors.high2]; edgcolr(2,2,:) = [colors.low2];
edgcolr(3,1,:) = [colors.high2]; edgcolr(3,2,:) = [colors.low2];
edgcolr(4,1,:) = [colors.high2]; edgcolr(4,2,:) = [colors.low2];
...
colr(1,1,:) = colors.high2; colr(1,2,:) = colors.low2;
colr(2,1,:) = colors.high2; colr(2,2,:) = colors.low2; 
colr(3,1,:) = colors.high2; colr(3,2,:) = colors.low2;
colr(4,1,:) = colors.high2; colr(4,2,:) = colors.low2;
edgcolr = nan(4,2,3);
edgcolr(1,1,:) = [colors.high2]; edgcolr(1,2,:) = [colors.low2];
edgcolr(2,1,:) = [colors.high2]; edgcolr(2,2,:) = [colors.low2];
edgcolr(3,1,:) = [colors.high2]; edgcolr(3,2,:) = [colors.low2];
...
superbar(1:4, firstasymbar,'E',firstasymstderr,'BarFaceColor', colr,...
    'BarEdgeColor',edgcolr,'ErrorbarStyle','|')
set(gca, 'XAxisLocation', 'bottom')
ylabel('steptime asymetry');
xticks(1:4)
xticklabels({'initial','early','late','plateau'})
title('steptime asymmetry in first exposure')