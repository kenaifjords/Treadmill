...
while i < min([length(hsfp1) length(hsfp2)])   
    ...
    if hsfp1(2)<hsfp2(2)   %fp1 strikes first
     steptime2(i)= hsfp2(i)-hsfp1(i);
    elseif hsfp1(2)>hsfp2(2) %fp2 strikes first so take its second heel strike
     steptime2(i)= hsfp2(i+1)-hsfp1(i); 
    end
 
    %step time on belt 1 (right)
     if hsfp2(2)<hsfp1(2)   %fp2 strikes first
...
% should i make steptime also correspond to time
subplot(212)
