...
for subj = 1:length(subject.list)
    ...
    for blk = 1:length(subject.blockname)
        ...
        [devicedata,trajdata] = readviconcsv_SC(subjpath,filename);
        if isempty(devicedata)
            % do nothing
            disp('readvicon output devicedata is empty')
        elseif isempty(trajdata)
            disp('readvicon output trajdata is empty')
        else
...
%                 dataraw = cat(2,dataraw,nan(size(dataraw,1), 30 - size(dataraw,2)));
        ...
        pp.trajtime{blk} = trajtime(itrim:end);
        pp.Lasis{blk} = trajdata(itrim:end,1:3);
        pp.Rasis{blk} = trajdata(itrim:end,4:6);
        pp.Lpsis{blk} = trajdata(itrim:end,7:9);
        pp.Rpsis{blk} = trajdata(itrim:end,10:12);
        pp.Lthigh{blk} = trajdata(itrim:end,13:15);
        pp.Lknee{blk} = trajdata(itrim:end,16:18);
        pp.Ltibia{blk} = trajdata(itrim:end,19:21);
        pp.Lankle{blk} = trajdata(itrim:end,22:24);
        pp.Lheel{blk} = trajdata(itrim:end,25:27);
        pp.Ltoe{blk} = trajdata(itrim:end,28:30);
        pp.Rthigh{blk} = trajdata(itrim:end,31:33);
        pp.Rknee{blk} = trajdata(itrim:end,34:36);
        pp.Rtibia{blk} =trajdata(itrim:end,37:39);
        pp.Rankle{blk} = trajdata(itrim:end,40:42);
        pp.Rheel{blk} = trajdata(itrim:end,43:45);
        pp.Rtoe{blk} = trajdata(itrim:end,46:48);
        end % isempty
    end %end blk
    cd([homepath '\matData'])
